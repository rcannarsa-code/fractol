...
static int check_fractal_type(char *type, t_fractal *fractal)
{
    ...
    else if (ft_strncmp(type, "newton", 6) == 0)
        fractal->fractal_type = 4;
    else if (ft_strncmp(type, "tricorn", 7) == 0)
        fractal->fractal_type = 5;
    else if (ft_strncmp(type, "sierpinski", 10) == 0)
        fractal->fractal_type = 6;
    else if (ft_strncmp(type, "lambda", 6) == 0)
        fractal->fractal_type = 7;
    else if (ft_strncmp(type, "celtic", 6) == 0)
        fractal->fractal_type = 8;
    else
        return (1);
    return (0);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
    {
        ft_printf("Usage: %s [mandelbrot/julia/burning_ship/newton/tricorn/sierpinski/lambda/celtic]\n", argv[0]);
        return (1);
    }
